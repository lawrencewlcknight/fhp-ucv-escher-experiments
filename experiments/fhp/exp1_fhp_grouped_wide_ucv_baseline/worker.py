"""Train one seed of the FHP Experiment 1 grouped-wide baseline."""
...
def _sync_remote(worker_dir: Path) -> None:
    ...
    if (
        ...
        raise ValueError(f"Invalid Experiment 1 remote task URI: {remote!r}")
    command = [
        "gcloud",
        "storage",
        "rsync",
...
def _validate_loaded_policy(checkpoint_path: Path) -> None:
    ...
    policy = LoadedFHPPolicy(game, checkpoint_path)
    probabilities = policy.action_probabilities(state)
    if not probabilities or not np.isclose(sum(probabilities.values()), 1.0):
        raise RuntimeError("Reloaded policy does not define a valid distribution")
    ...
    *,
    ...
    def capture_checkpoint(active_solver, raw_checkpoint):
        ...
        if int(payload["nodes_touched"]) != int(active_solver.nodes_touched):
            raise RuntimeError("Saved policy node count differs from solver state")
        record = {
        ...
        LOGGER.info(
            ...
            active_solver._training_elapsed_seconds() / 3600.0,
        )

    solver._post_checkpoint_callback = capture_checkpoint
    try:
        if len(captured) == len(schedule):
            checkpoint_rows = list(solver.checkpoint_rows)
            solver.stop_reason = "training_time_budget"
        else:
            checkpoint_rows = solver.solve(post_checkpoint_callback=capture_checkpoint)
        ordered = [captured[str(row["checkpoint_id"])] for row in schedule]
        if len(ordered) != 4 or solver.stop_reason != "training_time_budget":
            raise RuntimeError("Experiment 1 did not complete all four checkpoints")
