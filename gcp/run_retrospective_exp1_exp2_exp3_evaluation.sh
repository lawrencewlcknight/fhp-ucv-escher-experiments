...
BUILDER="$SCRIPT_DIR/retrospective_exp1_exp2_exp3_evaluation_batch.py"

: "${EXP1_RUN_ID:?Set EXP1_RUN_ID to the completed Experiment 1 run ID}"
...
: "${EXP23_EVAL_RUN_ID:?Set EXP23_EVAL_RUN_ID to the completed Experiment 2/3 evaluation run ID}"

if [[ "$ACTION" == "smoke-local" ]]; then
  EXP1_LOCAL_RUN="${EXP1_LOCAL_RUN:-$REPO_DIR/cloud_outputs/$EXP1_RUN_ID}"
  ...
  REFERENCE_LOCAL_ANALYSIS="${REFERENCE_LOCAL_ANALYSIS:-$REPO_DIR/cloud_outputs/$EXP23_EVAL_RUN_ID/analysis}"
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/fhp-exp1-exp2-exp3-retrospective-smoke}"
  cd "$REPO_DIR"
  exec python3 -m experiments.fhp.retrospective_exp1_exp2_exp3_evaluation.run \
    --exp1-run "$EXP1_LOCAL_RUN" --exp2-run "$EXP2_LOCAL_RUN" \
    --exp3-run "$EXP3_LOCAL_RUN" --reference-analysis "$REFERENCE_LOCAL_ANALYSIS" \
...
RUN_ID="${RUN_ID:-fhp-eval123-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/fhp-retrospective-eval-123.XXXXXX)"
trap 'rm -rf "$TEMP_DIR"' EXIT
JOB_JSON="$TEMP_DIR/job.json"

python3 "$BUILDER" --output "$JOB_JSON" --run-id "$RUN_ID" \
  --exp1-run-id "$EXP1_RUN_ID" --exp2-run-id "$EXP2_RUN_ID" \
  --exp3-run-id "$EXP3_RUN_ID" --reference-run-id "$EXP23_EVAL_RUN_ID" \
  --bucket-root "$BUCKET_ROOT" --service-account "$SA_EMAIL" --repo-ref "$REPO_REF"

preflight_service_account() {
  local described_email
  if ! described_email="$(gcloud iam service-accounts describe "$SA_EMAIL" \
...
preflight_inputs() {
  ...
  for experiment in exp1 exp2 exp3; do
    case "$experiment" in
      exp1) run_id="$EXP1_RUN_ID" ;;
      exp2) run_id="$EXP2_RUN_ID" ;;
      exp3) run_id="$EXP3_RUN_ID" ;;
    esac
    if ! gcloud storage ls "$BUCKET_ROOT/$run_id/workers/**/checkpoint_manifest.json" >/dev/null 2>&1; then
      echo "No checkpoint manifests found for $experiment at $BUCKET_ROOT/$run_id/workers" >&2
      return 2
    fi
  done
  if ! gcloud storage ls "$BUCKET_ROOT/$EXP23_EVAL_RUN_ID/analysis/evaluation_manifest.json" >/dev/null 2>&1; then
    echo "Missing Experiment 2/3 reference analysis at $BUCKET_ROOT/$EXP23_EVAL_RUN_ID/analysis" >&2
    return 2
  fi
}
...
case "$ACTION" in
  run)
    preflight_service_account
    preflight_inputs
    gcloud batch jobs submit "$RUN_ID" --project "$PROJECT_ID" \
      --location "$REGION" --config "$JOB_JSON"
    echo "Submitted Experiment 1--3 retrospective evaluation: $RUN_ID"
  ...
  status)
    ...
    ;;
  dry-run)
    cp "$JOB_JSON" "$REPO_DIR/retrospective_exp1_exp2_exp3_evaluation_job.json"
    echo "Wrote $REPO_DIR/retrospective_exp1_exp2_exp3_evaluation_job.json"
    ;;
  *)
