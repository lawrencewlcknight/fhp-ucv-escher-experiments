...
BUILDER="$SCRIPT_DIR/exp1_grouped_wide_batch.py"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/exp1-fhp-grouped-wide-smoke}"
  cd "$REPO_DIR"
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 1 commit SHA}"

RUN_ID="${RUN_ID:-exp1-fhp-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp1-fhp-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp1_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp1_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp1_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp1_aggregate_job.json"
    ;;
  status)
    gcloud batch jobs list --project "$PROJECT_ID" --location "$REGION" \
  ...
  run|resume)
    ...
    echo "Remote Experiment 1 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP1_REMOTE_CONTROLLER:-}" == "1" ]] || {
