#!/usr/bin/env python3
"""Build the single-VM Batch job for retrospective FHP evaluation."""
...
WORK_ROOT=/workspace/fhp-retrospective-evaluation
REPOSITORY="$WORK_ROOT/repository"
INPUT_ROOT="$WORK_ROOT/input"
OUTPUT_ROOT="$WORK_ROOT/output"
VENV_ROOT=/tmp/fhp-retrospective-evaluation-venv
...
mkdir -p "$WORK_ROOT" "$INPUT_ROOT/exp2" "$INPUT_ROOT/exp3" "$OUTPUT_ROOT" \
  "$UV_CACHE_DIR" "$UV_PYTHON_INSTALL_DIR" "$UV_INSTALL_DIR" "$MPLCONFIGDIR"
...
python -m experiments.fhp.retrospective_exp2_exp3_evaluation.run \
  --exp2-run "$INPUT_ROOT/exp2" \
  --exp3-run "$INPUT_ROOT/exp3" \
  --output-dir "$OUTPUT_ROOT/smoke" \
  --workers 2 --rule-deals 4 --lbr-deals 2 --lbr-rollouts 16 \
  --lbr-shard-deals 2 --crossplay-deals 10 --smoke

python -m experiments.fhp.retrospective_exp2_exp3_evaluation.run \
  --exp2-run "$INPUT_ROOT/exp2" \
  --exp3-run "$INPUT_ROOT/exp3" \
  --output-dir "$OUTPUT_ROOT/analysis" \
  --workers 8 --rule-deals {int(args.rule_deals)} \
  --lbr-deals {int(args.lbr_deals)} --lbr-rollouts {int(args.lbr_rollouts)} \
...
def build_job(args) -> dict:
    return {
        ...
        "labels": {"workload": "fhp-retrospective-eval", "stage": "analysis"},
    }
