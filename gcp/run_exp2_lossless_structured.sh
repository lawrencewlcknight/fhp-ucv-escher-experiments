...
BUILDER="$SCRIPT_DIR/exp2_lossless_structured_batch.py"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/exp2-fhp-lossless-structured-smoke}"
  cd "$REPO_DIR"
  exec python3 -m experiments.fhp.exp2_fhp_lossless_structured_ucv.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 2 commit SHA}"

RUN_ID="${RUN_ID:-exp2-fhp-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp2-fhp-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp2_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp2_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp2_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp2_aggregate_job.json"
    ;;
  status)
    gcloud batch jobs list --project "$PROJECT_ID" --location "$REGION" \
  ...
  run|resume)
    ...
    echo "Remote Experiment 2 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP2_REMOTE_CONTROLLER:-}" == "1" ]] || {
